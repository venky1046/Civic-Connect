...
import { adminListComplaints, getCategories } from '../services/complaintService';
...
export default function AdminComplaints() {
  ...
  useEffect(() => {
    setLoading(true);
    adminListComplaints(Object.fromEntries(Object.entries(filters).filter(([, v]) => v)))
      .then(setComplaints)
    ...
    <AdminLayout>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-ink-500 mt-1">{complaints.length} results</p>
        </div>
      ...
      <div className="flex flex-wrap gap-3 mt-5">
        ...
        <select className="input-field w-auto" value={filters.priority} onChange={(e) => updateFilter('priority', e.target.value)}>
          <option value="">All Priorities</option>
          {PRIORITIES.map((p) => <option key={p} value={p}>{p}</option>)}
        </select>
        <select className="input-field w-auto" value={filters.status} onChange={(e) => updateFilter('status', e.target.value)}>
