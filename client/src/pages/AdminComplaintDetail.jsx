...
export default function AdminComplaintDetail() {
  ...
  const handleUpdate = async (force = false) => {
    setSaving(true);
    try {
      await adminUpdateStatus(complaintId, {
        status,
        remark,
        confirmSkip: force,
      });
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          <div className="card p-6">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <h1 className="text-xl font-semibold text-navy-800 mt-0.5">
                  {complaint.title}
                </h1>
            ...
            <div className="grid sm:grid-cols-3 gap-4 mt-5 text-sm">
              ...
              <InfoRow
                ...
                value={`${complaint.communityCount} citizens`}
              />
            </div>

            {complaint.imagePath && (
              <img
                src={getImageUrl(complaint.imagePath)}
                alt="Reported issue"
                className="w-full h-64 object-cover rounded-lg mt-5 border border-line"
              />
            )}
          </div>
          ...
          <div className="card p-6">
            <h3 className="font-semibold text-navy-800 mb-4">
              Supporting Citizens ({supporters.length})
            </h3>
            ...
            {complaint.status === "RESOLVED" && (
              <div className="mt-4 pt-4 border-t border-line flex gap-4 text-sm">
                <span className="text-teal-700">
                  👍 {feedbackStats.YES || 0} confirmed resolved
                </span>
                <span className="text-red-600">
                  👎 {feedbackStats.NO || 0} said not resolved
                </span>
              </div>
            )}
          </div>
          ...
          <div className="card p-6">
            ...
            <div className="flex flex-col gap-3">
              {history.map((h) => (
                <div
                  key={h.id}
                  className="text-sm flex flex-wrap items-center gap-2 border-b border-line pb-3 last:border-0"
                >
                  <StatusBadge status={h.new_status} />
                  <span className="text-ink-400">by {h.changed_by_name}</span>
                  <span className="text-ink-400">
                    &middot; {new Date(h.created_at).toLocaleString("en-IN")}
                  </span>
                  {h.remark && (
                    <p className="w-full text-ink-700 mt-1">{h.remark}</p>
                  )}
                </div>
              ))}
            </div>
